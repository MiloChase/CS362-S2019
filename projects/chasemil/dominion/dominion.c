...
int j;
  ...
  if ( (getCost(state->hand[currentPlayer][choice2]) + 2) > getCost(choice1) ) //bug: swapped choice1 and choice2
{
return -1;
}
...
// adventurer refactored and pulled out of cardEffect
int adventurerFunc(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus, int currentPlayer, int drawntreasure){
  int z = 0;
  int cardDrawn;
  int temphand[MAX_HAND];
    while(drawntreasure<2){
  if (state->deckCount[currentPlayer] <2){//if the deck is empty we need to shuffle discard and add to deck  //changed 1 to 2 so that it will shuffle early
...
//mine refactored and pulled out of cardEffect
int mineFunc(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus, int currentPlayer){
int j;
int i;
  j = state->hand[currentPlayer][choice1];  //store card we will trash
...
return -1;
  ...
  if ( (getCost(state->hand[currentPlayer][choice1]) + 4) > getCost(choice2) ) //changed +3 to +4 so you can buy more exspensive things than you should be able to
{
return -1;
}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    ...
    case mine:
    return mineFunc(card, choice1, choice2, choice3, state, handPos, bonus, currentPlayer);
